...
│   │   ├── managers.hpp    // 管理类
│   │   ├── models.hpp      // 数据模型 
│   │   └── unrollindex.hpp // 分块链表检索
│   ├── file_io
│   │   ├── cache.hpp       // 缓存
│   │   ├── journal.hpp     // 容错日志
│   │   └── memoryriver.hpp // 文件读写封装
│   ├── config.hpp          // 全局配置
│   └── utils.hpp           // 异常类
└── src/
...
* **具体实现** ：
  ...
  * **JournalManager**:用于实现容错的类，记录了数据提交的信息，用于恢复

## 模块详细设计

### config
在config.hpp中定义了些需要使用的常数
```
//权限等级
...
### Expect类
用于验证合法性（以及拿bonus）的类，可以使用以下判断条件
...
struct Book {
  char isbn_[21];            //书籍编号
  char bookname_[61];        //书名
  char author_[61];          //作者名
  char keyword_[61];         //关键字
  int quantity_ = 0;         //库存
  double price_ = 0;         //单价
  double total_cast_ = 0;    //交易总额
};
```
...
```
struct User {
  int privilege_ = 0;    //权限等级
  char userid_[31];      //用户id
  char password_[31];    //密码
  char username_[31];    //用户名
};
```
...
struct SystemLog {
  ...
  SystemLog() = default;
  char userid_[31];             //操作者id
  char action_[30];             //行为
  char target_[31];             //目标对象
  int quantity_ = 0; 
...
* 输出：财务报表
```
PrintStaff(UserManager);
```
* 输入：用户管理类，方便通过日志中的id对应的用户信息
...
### JournalManager
本类用于实现容错机制，旨在通过日志（不同于LogManager管理的日志）记录数据的提交和记录，在每次系统启动时检查日志文件，通过日志的内容可以判断该数据是否遭到损坏。
```
...
```
日志头，用于记录日志信息
* **type**:记录日志的类别，分为**READY, WRITE, FINISH**，代表日志提交的不同阶段，每个指令提交的日志应该至少由一个**READY**开头，一个**FINISH**结尾，代表指令执行的开始和结束。
* **fileID**：当**type**为**WRITE**时，fileID代表该数据提交的文件编号，文件编号在对应**MemoryRiver**实例初始化时决定，由于初始化顺序时固定的，所以文件标号时固定的。
* **offset**：相应数据在文件的offset
* **data_size**：对应数据的长度
...
```
记录了各个MemoryRiver实例的回调函数，分别用于在恢复数据的时候进行数据写回和重新整理文件信息
```
void AddLog(const JournalHeader &, const char *);
```
在文件日志中写入文件头和数据（如果不是**WRITE**则传入**nullptr**）
```
int Init(rawWrite, reFresh);
```
被各个MemoryRiver实例调用，提供其回调函数
...
void Finish();
```
分别是三个指令执行阶段进行日志提交，调用**AddLog**
其中**Write**输入文件编号、offset、数据长度以及数据在内存的地址
```
void Recover();
```
恢复函数，每次在程序开始时被调用，用于通过日志信息恢复数据以及数据是否被破坏
