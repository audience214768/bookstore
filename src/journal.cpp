...
void JournalManager::Write(int fileID, int offset, int data_size, const char *data) {
  ...
  if (data != nullptr && data_size > 0) {
    header.checksum = xcrc32(reinterpret_cast<const unsigned char*>(data), data_size, 0xFFFFFFFF);
  }
  AddLog(header, data);
}
...
void JournalManager::Recover() {
  ...
  while(file.peek() != EOF) {
    ...
    if(header.type == WRITE) {
      ...
      unsigned int calcCRC = xcrc32(reinterpret_cast<const unsigned char*>(data.data()), header.data_size, 0xFFFFFFFF);
      if (calcCRC != header.checksum) {
        std::cerr << "CRC Check Failed! Log corrupted." << std::endl;
        dataloss = 1;
        buffer.clear();
        break;
      }
